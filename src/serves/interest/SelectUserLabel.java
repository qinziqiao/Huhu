...
public class SelectUserLabel extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response){	
		...
		try {
			UserLabelMapTable uerLabelMapTable=new UserLabelMapTable(GlobalParameter.uri, GlobalParameter.sql_user, GlobalParameter.sql_password);
			ResultSet rs =uerLabelMapTable.getULMT(request_uid);
			//对结果集进行JSON解析
			if(rs.next()==false){
				//没有找到数据
				response.setStatus(500);
